...
describe("buildHostFilesystemUrl", () => {
  it("returns the no-path endpoint when absolutePath is empty", () => {
    ...
    expect(buildHostFilesystemUrl("host_abc", "")).toBe(
      "/v1/hosts/host_abc/filesystem",
    );
  });
  ...
  it("strips the single leading slash (FastAPI re-adds it)", () => {
    ...
    expect(
      buildHostFilesystemUrl("host_abc", "/Users/corey/foo"),
    ).toBe("/v1/hosts/host_abc/filesystem/Users/corey/foo");
  });
  ...
  it("encodes special characters per segment", () => {
    ...
    expect(
      buildHostFilesystemUrl("host_abc", "/Users/c o/foo bar"),
    ).toBe("/v1/hosts/host_abc/filesystem/Users/c%20o/foo%20bar");
  });
